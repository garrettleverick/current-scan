...
Int_t collect(){
    ...
    for(Int_t g=0; g<gen.size(); g++){
        TString in_name = Form("%s/current-scan_data/int_data1/%s.root", file_stem.Data(), gen[g].Data());
                ...
                for(Int_t q=0; q<p_nrg.size(); q++){
                    for(Int_t s=0; s<n_septant; s++){
                        for(Int_t r=0; r<n_ring+1; r++){
                            if(r!=0 && r!=5){ //for all rings exxept 0 (all) and 5 we are collecting the sectors into the tiles
